        ...
        let tok = match b[pos] as char {
            ...
            c if c.is_alphabetic() || c == '_' => {
                ...
                if word.eq_ignore_ascii_case("ORDER") {
                    ...
                    if input[s2..pos].eq_ignore_ascii_case("BY") {
                        Token::OrderBy
                    } else {
                        pos = save;
...
impl P {
    ...
    fn query(&mut self) -> Result<Query, String> {
        if !matches!(self.next(), Token::Select) {
            return Err("Expected SELECT".into());
        }
        if matches!(self.peek(), Token::Star | Token::Ident(_)) {
            self.next();
        }
        ...
        loop {
            match self.peek() {
                Token::Where => {
                    self.next();
                    q.filter = Some(self.filter()?);
                }
                Token::OrderBy => {
                    self.next();
                    q.order_by = Some(match self.next() {
                        Token::Ident(s) => s,
                        _ => return Err("Expected field".into()),
                    });
                }
                Token::Limit => {
                    self.next();
    ...
    fn filter(&mut self) -> Result<Filter, String> {
        ...
        match self.peek() {
            Token::And => {
                self.next();
                Ok(Filter::And(Box::new(left), Box::new(self.filter()?)))
            }
            Token::Or => {
                self.next();
                Ok(Filter::Or(Box::new(left), Box::new(self.filter()?)))
            }
            _ => Ok(left),
        }
    }
    ...
    fn atom(&mut self) -> Result<Filter, String> {
        if matches!(self.peek(), Token::Not) {
            self.next();
            return Ok(Filter::Not(Box::new(self.atom()?)));
        }
...
mod tests {
    ...
    #[test]
    fn basic() {
        let q = parse("SELECT * COMMANDS WHERE frequency > 5 LIMIT 10").unwrap();
        assert!(matches!(q.source, Source::Commands));
        assert_eq!(q.limit, Some(10));
    }
    #[test]
    fn workflows() {
        let q = parse("SELECT * WORKFLOWS").unwrap();
        assert!(matches!(q.source, Source::Workflows));
        assert!(q.filter.is_none());
    }
    #[test]
    fn and_filter() {
        let q = parse("SELECT * COMMANDS WHERE frequency > 3 AND score > 10").unwrap();
        assert!(matches!(q.filter, Some(Filter::And(_, _))));
    }
    #[test]
    fn order() {
        let q = parse("SELECT * COMMANDS ORDER BY frequency LIMIT 5").unwrap();
        assert_eq!(q.order_by.as_deref(), Some("frequency"));
    }
}
