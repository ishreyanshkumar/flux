    ...
    if let Event::Key(k) = event::read()? {
        ...
        match app.mode.clone() {
            ...
            Mode::RemoveAlias => text_input_keys(app, k.code, |a| {
                if !a.input.is_empty() {
                    a.confirm_yes = true;
                    a.mode = Mode::Confirm(Action::RemoveAlias {
                        alias: a.input.clone(),
                    });
                }
            }),
...
fn main_keys(app: &mut App, code: KeyCode) {
    match code {
        ...
        KeyCode::Char('r') => {
            app.input.clear();
            app.mode = Mode::RemoveAlias;
        }
        KeyCode::Char('c') => {
            app.input.clear();
            app.mode = Mode::ChangeAlias;
        }
        KeyCode::Char('l') => {
            app.reload_aliases();
...
fn search_keys(app: &mut App, code: KeyCode) {
    match code {
        ...
        KeyCode::Up => {
            app.search_state
                .select(app.search_state.selected().map(|i| i.saturating_sub(1)));
        }
        _ => {}
    }
}
    ...
    match code {
        ...
        KeyCode::Tab => {
            if !app.suggestions.is_empty() {
                app.mode = Mode::PickSuggestion;
            }
        }
        KeyCode::Up | KeyCode::Down => {
            app.add_alias_focus_command = !app.add_alias_focus_command;
        }
        KeyCode::Backspace => {
            if app.add_alias_focus_command {
                app.input.pop();
            } else {
                app.alias_input.pop();
            }
        ...
        KeyCode::Enter => {
            if app.input.is_empty() || app.alias_input.is_empty() {
                app.status = "Fill in both command and alias.".into();
                return;
            }
            app.confirm_yes = true;
            app.mode = Mode::Confirm(Action::AddAlias {
                alias: app.alias_input.clone(),
...
fn pick_sug_keys(app: &mut App, code: KeyCode) {
    match code {
        ...
        KeyCode::Up => {
            app.sug_state
                .select(app.sug_state.selected().map(|i| i.saturating_sub(1)));
        }
        KeyCode::Enter => {
            if let Some(i) = app.sug_state.selected() {
                if let Some(s) = app.suggestions.get(i) {
...
fn list_keys(app: &mut App, code: KeyCode) {
    match code {
        ...
        KeyCode::Down => {
            let n = app.aliases.len();
            app.alias_state.select(
                app.alias_state
                    .selected()
                    .map(|i| (i + 1).min(n.saturating_sub(1))),
            );
        ...
        KeyCode::Up => {
            app.alias_state
                .select(app.alias_state.selected().map(|i| i.saturating_sub(1)));
        }
        _ => {}
    }
}
