...
fn render_search(f: &mut Frame, app: &App, area: Rect) {
    ...
    f.render_stateful_widget(
        List::new(items)
            ...
            .highlight_symbol("▶ "),
        chunks[1],
        &mut app.search_state.clone(),
    );
    f.render_widget(
        status("Type to search  •  ↑↓ navigate  •  Esc back"),
        chunks[2],
    );
}
...
fn render_add_alias(f: &mut Frame, app: &App, area: Rect) {
    ...
    let cmd_color = if app.add_alias_focus_command {
        ACCENT
    } else {
        DIM
    };
    let alias_color = if !app.add_alias_focus_command {
        ACCENT
    } else {
        DIM
    };
    ...
    f.render_widget(
        Paragraph::new(app.alias_input.as_str()).block(
            Block::default()
                .borders(Borders::ALL)
                .border_type(BorderType::Rounded)
                .title(Span::styled(
                    " Alias  (Tab → suggestions) ",
                    Style::default()
                        .fg(alias_color)
                        .add_modifier(Modifier::BOLD),
                )),
    ...
    f.render_widget(
        status("Type  •  ↑↓ switch boxes  •  Tab pick  •  Enter confirm  •  Esc back"),
        chunks[3],
    );
}
...
fn render_remove_alias(f: &mut Frame, app: &App, area: Rect) {
    ...
    f.render_widget(
        status("Type alias name  •  Enter confirm  •  Esc back"),
        bar,
    );
}
...
fn render_change_alias(f: &mut Frame, app: &App, area: Rect) {
    let [hint, inp, bar] = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(3),
            Constraint::Length(3),
    ...
    f.render_widget(
        Paragraph::new("old_alias  new_alias  command")
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .border_type(BorderType::Rounded)
    ...
    f.render_widget(
        status("old_alias new_alias command  •  Enter confirm  •  Esc back"),
        bar,
    );
}
...
fn render_list_aliases(f: &mut Frame, app: &App, area: Rect) {
    ...
    f.render_stateful_widget(
        List::new(items)
            .block(block(&format!("Aliases ({})", app.aliases.len())))
            .highlight_style(Style::default().bg(Color::Rgb(40, 44, 52)))
            .highlight_symbol("▶ "),
        list,
        &mut app.alias_state.clone(),
    );
...
fn render_query(f: &mut Frame, app: &App, area: Rect) {
    ...
    f.render_widget(
        Paragraph::new(app.query_input.as_str()).block(
            Block::default()
                .borders(Borders::ALL)
                .border_type(BorderType::Rounded)
                .title(Span::styled(
