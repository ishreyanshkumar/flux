...
pub fn init_script(shell: &Shell, cfg: &Config) -> String {
    let bin = std::env::current_exe()
        .unwrap_or_else(|_| "flux".into())
        .to_string_lossy()
        .to_string();
    let aliases = cfg.primary_alias_file();
    match shell {
        Shell::Bash => format!(
            r#"# Flux — bash. Add to ~/.bashrc: eval "$({bin} init bash)"
...
"#
        ...
        Shell::Zsh => format!(
            r#"# Flux — zsh. Add to ~/.zshrc: eval "$({bin} init zsh)"
...
"#
        ...
        Shell::Fish => format!(
            r#"# Flux — fish. Add to config.fish: {bin} init fish | source
...
"#
        ...
        Shell::Posix => format!(
            r#"# Flux — POSIX. Add to ~/.profile.
[ -f "{aliases}" ] && . "{aliases}"
"#
        ),
        Shell::Powershell => format!(
            r#"# Flux — PowerShell. Add to $PROFILE: Invoke-Expression (& "{bin}" init powershell | Out-String)
$global:FluxBin = "{bin}"
$FluxAliases = "{aliases}"
