...
pub fn sessionize(events: &[ShellEvent], gap_secs: i64) -> Vec<Session> {
    ...
    for ev in events {
        let new = cur.as_ref().map_or(true, |s| {
            ev.session_id != s.id
                || ev.timestamp - s.events.last().map_or(0, |e| e.timestamp) > gap_secs
        });
        if new {
...
pub struct WorkflowDag {
    adj: HashMap<String, HashMap<String, usize>>,
}
...
impl WorkflowDag {
    ...
    pub fn ingest(&mut self, sessions: &[Session]) {
        for s in sessions {
            for w in s.events.windows(2) {
                *self
                    .adj
                    .entry(w[0].command.clone())
                    .or_default()
                    .entry(w[1].command.clone())
                    .or_insert(0) += 1;
            }
        }
    }
    ...
    pub fn predict(&self, current: &str, n: usize) -> Vec<(String, f64)> {
        let Some(ns) = self.adj.get(current) else {
            return vec![];
        };
        let total: usize = ns.values().sum();
        let mut v: Vec<(String, f64)> = ns
            .iter()
            .map(|(cmd, &c)| (cmd.clone(), c as f64 / total as f64))
            .collect();
...
pub fn mine_workflows(sessions: &[Session], min_freq: usize) -> Vec<(Vec<String>, usize)> {
    ...
    let mut filtered: Vec<(Vec<String>, usize)> = ngrams
        .into_iter()
...
pub fn compute_stats(records: &[&Record], existing_alias_cmds: &[String]) -> Stats {
    ...
    let total_keystrokes = records
        .iter()
        .map(|r| r.char_len as usize * r.frequency as usize)
        .sum();
    let mut candidates: Vec<(String, i32, usize)> = records
        .iter()
        ...
        .map(|r| {
            (
                r.command.clone(),
                r.frequency,
                (r.char_len as usize).saturating_sub(2) * r.frequency as usize,
            )
        })
        .collect();
