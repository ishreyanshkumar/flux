...
impl Trie {
    pub fn insert(&mut self, text: &str, id: u32) {
        ins(&mut self.root, text.as_bytes(), id);
    }
    pub fn prefix(&self, q: &str) -> Vec<u32> {
        let mut out = vec![];
        collect(&self.root, q.as_bytes(), &mut out);
        out
    }
}

fn cp(a: &[u8], b: &[u8]) -> usize {
...
fn collect(node: &Node, key: &[u8], out: &mut Vec<u32>) {
    ...
    if let Some(e) = node.children.get(&key[0]) {
        let c = cp(key, &e.label);
        if c == key.len() {
            collect_all(&e.node, out);
        } else if c == e.label.len() {
            collect(&e.node, &key[c..], out);
        }
    }
}
...
impl Bm25 {
    ...
    fn add(&mut self, id: u32, text: &str) {
        ...
        for t in toks {
            *self.inv.entry(t).or_default().entry(id).or_insert(0.0) += 1.0;
        }
    }
...
impl SearchEngine {
    pub fn new(cfg: &Config) -> Self {
        SearchEngine {
            trie: Trie::default(),
            bm25: Bm25::new(cfg.bm25_k1, cfg.bm25_b),
            docs: vec![],
    ...
    pub fn index(&mut self, rec: &Record) {
        if self.text_to_id.contains_key(&rec.command) {
            return;
        }
    ...
    pub fn search(&self, query: &str, limit: usize) -> Vec<SearchResult> {
        ...
        let mut out: Vec<SearchResult> = self
            ...
            .filter_map(|(idx, rec)| {
                ...
                let is_fuzzy =
                    !is_prefix && bm25 == 0.0 && fuzzy_match(query, &rec.command, max_dist);
                if !is_prefix && bm25 == 0.0 && !is_fuzzy {
                    return None;
                }
                Some(SearchResult {
...
impl AliasSuggester {
    pub fn new(existing_aliases: Vec<String>) -> Self {
        AliasSuggester {
            existing: existing_aliases.into_iter().collect(),
    ...
    pub fn suggest(&self, command: &str) -> Vec<Suggestion> {
        ...
        out.sort_by_key(|b| std::cmp::Reverse(self.priority(b)));
        out
    }
    ...
    fn priority(&self, s: &Suggestion) -> i32 {
        let base = if ["Git", "Docker", "Cargo", "NPM", "Kubectl"]
            .iter()
            .any(|p| s.reason.starts_with(p))
        {
            100
        } else if s.reason == "Abbreviation" {
            90
        } else if s.reason == "Vowel removal" {
            80
        } else if s.reason.contains("combination") {
            70
        } else {
            40
        };
    ...
    fn vowel_removal(&self, command: &str) -> Vec<Suggestion> {
        let s: String = command
            ...
            .take(8)
            .collect();
        if s.len() >= 2 && s != command {
            vec![sg(&s, command, "Vowel removal")]
        } else {
            vec![]
        }
    }
    ...
    fn combined(&self, command: &str) -> Vec<Suggestion> {
        ...
        vec![sg(
            &a,
            command,
            &format!("{}-{} combination", parts[0], parts[1]),
        )]
    }
    ...
    fn truncated(&self, command: &str) -> Vec<Suggestion> {
        ...
        (2..=tool.len().min(5))
            .filter_map(|len| {
                ...
                if t != tool {
                    Some(sg(&t, command, &format!("Truncated to {} chars", len)))
                } else {
                    None
                }
            })
